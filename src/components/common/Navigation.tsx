...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent) {
      if (popoverElement) {
        const isClickInsidePopover = popoverElement.contains(
          event.target as Node
        );
        if (!isClickInsidePopover) {
          const activePopover = popoverElement.querySelector(
            '[aria-expanded="true"]'
          );
          if (activePopover) {
                ...
                <div className="grid">
                  {links.map((link) => (
                    <Link
                      ...
                      variant="ghost"
                      className={clsx(
                        "rounded-md px-4 py-2 text-black no-underline transition-colors dark:text-white",
                        pathname === link.href
