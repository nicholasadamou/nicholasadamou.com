...
> **Note about the Demo**: While a [live demo](https://cloud-tools.vercel.app) is available, the backend AWS infrastructure is not currently deployed to keep costs minimal. The demo showcases the frontend interface and user experience, but file processing functionality won't work. However, you can experience the full functionality locally using LocalStack by following the setup instructions in the [GitHub repository](https://github.com/nicholasadamou/cloud-tools).
...
const publishMetrics = async (metricName: string, value: number, unit: string) => {
