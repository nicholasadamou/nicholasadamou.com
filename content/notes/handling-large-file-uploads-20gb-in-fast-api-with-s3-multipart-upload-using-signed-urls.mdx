...
Ever tried uploading massive files (20GB–30GB) through your backend server? Memory spikes, timeouts, server crashes—it's a mess. That's why you need **Amazon S3 Multipart Upload with Signed URLs**.
...
1. **Client → API**: POST `/api/start-upload`
2. **API → S3**: Create multipart upload → Get `upload_id`
3. **API → MongoDB**: Record upload session
4. **API → Client**: Return `upload_id`, `key`
...
- Client → S3: PUT part directly using `signed_url` → Get `ETag`
- Client → API: POST `/api/upload-part` with `ETag`
- API → MongoDB: Record part info

**Complete upload:**

- Client → API: POST `/api/complete-upload`
- API → MongoDB: Get all parts
- API → S3: Complete multipart upload → Get file location
- API → MongoDB: Update status to completed
- API → Client: Return success + file location
