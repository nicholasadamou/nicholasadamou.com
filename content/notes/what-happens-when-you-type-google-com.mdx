...
summary: "My experience being asked this classic interview question — a complete walkthrough of the networking journey from browser to server and why it matters for DevOps."
date: "2026-01-25"
pinned: false
...
That's when it clicked. This wasn't about reciting a memorized answer — it was about demonstrating that I understood every layer of the network stack and, more importantly, where I could intervene as a DevOps engineer.
...
## The Complete Journey (My Answer)
...
2. **Recursive DNS query**:
   - Local DNS resolver → Root DNS servers
   - Root servers → `.com` TLD servers
   - TLD servers → Google's authoritative nameservers
...
- If external → send to **default gateway** (your router)
...
This creates a DNS entry in the cluster that routes `google-proxy.default.svc.cluster.local` → `google.com`.
